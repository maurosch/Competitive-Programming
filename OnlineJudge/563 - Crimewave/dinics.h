 ...
 * lo cual es O(n^{2/3}m) para caso denso O(m^{3/2}) para ralos.
...
public:
     ...
     * Precondicion: hay cargado un flujo maximo de source a algun sink.  
     * Caso contrario, puede retornar c[v] = true para todo v.
     */
    std::vector<bool> st_cut(Vertex source) const {
     ...
     */
    std::vector<edge> st_cut_edges(Vertex source) {
        std::vector<edge> res;

        auto cut = st_cut(source);
        for(auto& N : residual)
        for(auto& e : N)
        if(epsilon_bigger(e.capacity, 0))
